...
public class Main {
    ...
    static final String FILE = "division";

    public void solve() {
        long n = in.nextLong();
        long mid = max(1L, n / 3 - 10);
        long mini = Long.MAX_VALUE, aa = 0, bb = 0, cc = 0;
        for (long a = mid; a <= mid + 20; a++) {
            for (long b = a + 1; b <= mid + 20; b++) {
                for (long c = b + 1; c <= mid + 20; c++) {
                    if (a + b + c != n)
                        continue;
                    long curr = a * a + b * b + c * c;
                    if (curr < mini) {
                        mini = curr;
                        aa = a;
                        bb = b;
